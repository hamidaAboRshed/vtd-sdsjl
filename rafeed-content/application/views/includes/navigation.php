        ...
        <?php 
          foreach ($this->session->userdata('navdata') as $key => $value) {
              echo '<li class="nav-item '. ($value["active"]==1? "active":"") .'" >
                  <a class="nav-link text-white link display-4 nav-link-'.$value['Name'].'" href="'.$this->navigation->get_base_url().'/Product/Product_series_list/'.$value["ID"].'" >'.
        				  '<span class="ac-gn-link-text hide">'.$value['Name'].'</span>'.
        				  '</a></li>';
                }
          ?>
      </ul>
